...
public final class AlcoholHandler {
    ...
    public static void increaseInebriation(LivingEntity entity, int level) {
        ...
        if (entity.hasEffect(INEBRIATION)) {
            ...
            entity.addEffect(new MobEffectInstance(INEBRIATION, Math.max(currentDuration + (INEBRIATION_DURATION * level), 0), Math.max(currentAmplifier + level, 0), false, false));
        } else if (level >= 1) { // If the Entity is not already inebriated and we are attempting to add levels
            entity.addEffect(new MobEffectInstance(INEBRIATION, INEBRIATION_DURATION * level, level - 1, false, false));
        };
    }
};
